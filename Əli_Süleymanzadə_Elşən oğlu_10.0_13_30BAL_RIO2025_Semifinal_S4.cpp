...
void _(){
    ...
    d(v);
    map<ll, mll> memo;
    for(ll i=len(v)-1;i>=0;i--){
        mll p;
        p[v[i].sc] = v[i].fr;
        ll mxi=v[i].fr, mni = v[i].fr, mn = INF;
        for(ll j=i+1;j<len(v);j++){
            if(len(memo[j]) == n or (len(memo[j]) == n-1 and !memo[j].count(v[i].fr))){
                mxi=v[i].fr, mni=v[i].fr;
                for(auto x : memo[j]){
                    if(x.fr == v[i].sc) continue;
                    mxi = max(mxi, x.sc);
                    mni = min(mni, x.sc);
                }
                if(abs(mxi-mni) < mn){
                ...
                if(len(p) == n){ mn=min(mn, mxi-mni); d(mn, p); break;}
            }
        }
        if(len(p) == n) cv = min(cv, mn);
        d(p);
        memo[i] = p;
    }
    cout<<cv<<'\n';
